...
```
<build_folder>/zilk_core/dev/cli/embed_toml -i third_party/erigon-snapshot -o silkworm/db/datastore/snapshots/config/chains
```
...
all the currently supported configurations looking at `genesis_<network>.json` files in `zilk_core/core/chain` folder.

If you need to expand or modify the network configurations used by Silkworm, the following procedure must be applied:

1. add new or edit existing JSON genesis files in `zilk_core/core/chain` following the naming convention `genesis_<network>.json`
2. generate the C++ code bindings for JSON genesis files by executing from project home folder:
```
<build_folder>/zilk_core/dev/cli/embed_json -i zilk_core/core/chain -o zilk_core/core/chain -w
```
...
```
<build_folder>/zilk_core/dev/cli/embed_json -i silkworm/rpc/json_rpc -o silkworm/rpc/json_rpc -p specification -n silkworm::rpc::json_rpc -w
```
